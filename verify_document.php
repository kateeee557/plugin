...
/**
 * Document verification page for the blockchain module.
 *
...
$document = $service->get_document($documentid);

$PAGE->set_url('/mod/blockchain/verify_document.php', ['id' => $documentid, 'moduleid' => $moduleid]);
$PAGE->set_title(get_string('verifydocument', 'mod_blockchain'));
$PAGE->set_heading(get_string('pluginname', 'mod_blockchain'));
$PAGE->set_context($context);

$renderer = $PAGE->get_renderer('mod_blockchain');
echo $OUTPUT->header();
echo $renderer->render_verify_document(['filename' => $document->filename, 'id' => $document->id, 'verified' => $document->token_id, 'moduleid' => $moduleid]);
echo $OUTPUT->footer();