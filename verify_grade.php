...
/**
 * Grade verification page for the blockchain module.
 *
...
$service = new mod_blockchain_grade_service($moduleid);
$grade = $service->get_grade($gradeid);

$PAGE->set_url('/mod/blockchain/verify_grade.php', ['id' => $gradeid, 'moduleid' => $moduleid]);
$PAGE->set_title(get_string('verifygrade', 'mod_blockchain'));
$PAGE->set_heading(get_string('pluginname', 'mod_blockchain'));
$PAGE->set_context($context);

$renderer = $PAGE->get_renderer('mod_blockchain');
echo $OUTPUT->header();
echo $renderer->render_verify_grade(['submissionid' => $grade->submissionid, 'grade' => $grade->grade, 'verified' => $grade->tx_hash, 'moduleid' => $moduleid]);
echo $OUTPUT->footer();